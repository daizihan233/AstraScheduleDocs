...
开关控制客户端左侧信息栏的显示与隐藏。左侧信息栏展示温度、天气和星期标签。关闭后左侧栏整体隐藏，课表主体仍然居中显示。
...
- **是否使用简略的预警信息**：开启后使用简短描述（如"江苏省气象台发布大雾黄色预警"），关闭则显示完整预警内容。

## 天气温度颜色配置

控制客户端上温度显示的颜色方案：

- **启用颜色渐变**：关闭时为离散色块模式（温度落入哪个区间就用该区间的颜色，推荐），开启时为渐变色模式（在相邻端点间线性过渡）。
...
键值对形式自定义客户端 CSS 变量，支持动态添加/删除条目。每行包含变量名和变量值。客户端将应用这些变量，实现高度灵活的界面定制。不过，我们不建议您随便改。

## 提交前预览

底部 JSON 预览区域实时展示完整的配置数据结构。通常您不用关心。
